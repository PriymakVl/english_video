...
{
    public function actionCreateFileStrings($type, $text_id)
    {
        if ($type == TYPE_WORD) $items = Word::findAll(['sound_id' => null, 'status' => STATUS_ACTIVE]);
        else  $items = Phrase::findAll(['sound_id' => null, 'text_id' => $text_id, 'status' => STATUS_ACTIVE]);
